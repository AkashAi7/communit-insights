...
teamsApp.on('message', async (context) => {
  ...
  if (["hi", "hello", "hey"].includes(lowerText)) {
    await context.send(
      ...
      "I can help you discover actionable insights from developer feedback, " +
      "summarize pain points, and answer your questions about the Teams developer community.\n\n" +
      "Try commands like `/show_insights`, `/search_insights <topic>`, or just ask me about developer pain points!"
    );
    return;
  }

  // --- Friendly response for "what can you do" ---
  if (lowerText.includes("what can you do") || lowerText.includes("help")) {
    await context.send(
      "🤖 I analyze developer feedback from forums and surface actionable insights for the Teams Platform. " +
      "You can:\n" +
      "- Browse insights with `/show_insights`\n" +
      "- Search for issues with `/search_insights <keyword>`\n" +
      "- Ask about the latest or a specific insight\n" +
      "- Or just chat with me about developer pain points!\n\n" +
  ...
  if (lowerText.startsWith('/search_insights')) {
    ...
    // Search in summary and painPoints
