...
// Track which insight index each user/conversation is on
...
mcpExpressApp.post('/api/mcp/ingest', async (req, res) => {
    try {
        ...
        // --- Reset all user insight indices on new ingest ---
...
teamsApp.on('message', async (context) => {
  ...
  // Use conversation id as key (or aadObjectId if you want per-user)
  ...
  if (text === 'hello') {
      ...
      // Always start from the first insight
