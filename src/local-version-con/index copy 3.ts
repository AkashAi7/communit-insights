...
teamsApp.on('message', async (context) => {
  ...
  if (text === 'hello') {
      ...
      // Example: Mock feedback to show AI analysis within the bot
      const mockFeedbackToAnalyze = [
        { 
            id: 1001, 
            text: "The new authentication flow is causing issues for users on older mobile devices. Many are reporting login failures and slow response times.", 
            source: "Customer Support Tickets", 
            url: "http://example.com/tickets/1001" 
        }
      ];

       try {
          // Call the tool using the correct Teams AI SDK v2 API
  const analysisOutput = await analyzeFeedbackToolHandler({ 
    feedback: mockFeedbackToAnalyze
}, {});

          if (analysisOutput.analyzedResults && analysisOutput.analyzedResults.length > 0) {
              const firstAnalysis = analysisOutput.analyzedResults[0].analysis;
              const card = createFeedbackAnalysisCard(firstAnalysis);
              ...
              console.log('Sent Adaptive Card with analysis results to Teams.');
          } else {
              await context.send('Could not retrieve analysis results. The AI might not have returned content or encountered an error.');
          }

      } catch (error: any) {
          console.error('Error calling analyzeFeedback tool internally from bot:', error);
          await context.send(`Sorry, I encountered an error while analyzing feedback: ${error.message}`);
      }

  } else {
      await context.send(`You said: "${context.activity.text}". I can echo messages or show feedback insights if you type \`/show_insights\`.`);
  }
